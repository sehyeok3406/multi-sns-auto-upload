...
- Threads 설문 게시
- Threads 텍스트 첨부 게시
- Threads 블록별 이미지 첨부 및 Vercel Blob 업로드
- Threads 500자 제한, 450자 이상 경고, 500자 초과 게시 차단
- Threads 타래 작성 및 `reply_to_id` 기반 이어 붙이기
- Threads 텍스트/이미지 스포일러 게시
- `/dashboard/replies` Threads 댓글함: 내 게시물 조회, 댓글/대화 조회, 답글 게시
- `/dashboard/insights` Threads 게시 성과 조회
- 실시간 글자 수 표시
- X / Threads 업로드 대상 선택
- 게시글 미리보기
...
Threads API 연동은 `lib/publisher/threadsPublisher.ts`의 `publishToThreads`에서 처리합니다. 텍스트 게시 또는 단일 이미지 게시 기준으로 컨테이너 생성 후 publish를 호출합니다. 이미지 첨부 시 Vercel Blob에 업로드된 공개 URL을 Threads API의 `image_url`로 전달합니다. 주제 선택 시 Threads API의 `topic_tag` 파라미터로 함께 전달합니다. 설문 선택지는 Threads API의 `poll_attachment` 파라미터로 전달하며, 설문은 텍스트 게시 전용으로 이미지 첨부와 함께 사용할 수 없습니다. 긴 글은 Threads API의 `text_attachment` 파라미터로 전달하며, 텍스트 첨부도 설문 및 이미지 첨부와 함께 사용할 수 없습니다. 타래 작성 시 첫 게시물을 발행한 뒤 추가 글을 `reply_to_id`로 이어 붙이고, 스포일러는 컨테이너 생성 단계에서 `text_entities` 또는 `is_spoiler_media`로 전달합니다.
...
Threads 게시 성과는 `lib/publisher/threadsInsights.ts`에서 처리합니다. 최근 게시물 목록을 가져온 뒤 각 게시물의 `/{threads-media-id}/insights`를 호출해 `views`, `likes`, `replies`, `reposts`, `quotes`, `shares` 지표를 표시합니다. 게시 성과를 쓰려면 Threads 토큰에 `threads_manage_insights` 권한이 포함되어 있어야 합니다.
...
Threads 이미지 게시에는 공개 이미지 URL이 필요합니다. 이 프로젝트는 각 타래 블록의 이미지를 Vercel Blob Public Store에 업로드한 뒤 반환된 URL을 Threads API에 전달합니다.
...
게시글 작성 화면은 Threads 기준 500자 제한을 적용합니다. 450자부터 경고를 표시하고, 500자를 넘으면 게시 버튼과 서버 API 양쪽에서 차단합니다.

`스레드에 추가` 버튼으로 타래 글을 추가할 수 있습니다. 각 블록은 독립적으로 500자 제한을 검증하며, 실제 게시 시 첫 글을 발행한 뒤 다음 글부터 바로 앞 게시물의 `reply_to_id`로 연결합니다.

설문은 1번 글에 첨부할 수 있습니다. 선택지는 최소 2개, 최대 4개이며 각 선택지는 25자 이하입니다. Threads API 제한에 맞춰 설문을 켠 상태에서는 이미지 첨부를 함께 사용할 수 없습니다.

텍스트 첨부는 1번 글에 긴 글을 추가하는 기능입니다. 첨부 본문은 최대 10,000자이며 선택적으로 링크 URL을 함께 입력할 수 있습니다. Threads API 제한에 맞춰 텍스트 첨부는 설문 및 이미지 첨부와 함께 사용할 수 없습니다.

텍스트를 드래그한 뒤 `선택 영역 스포일러`를 누르면 해당 구간이 스포일러로 저장됩니다. 이미지 첨부 시에는 이미지를 스포일러로 표시하는 옵션도 사용할 수 있습니다.
...
게시물 작성은 `/dashboard`, 댓글 작성은 `/dashboard/replies`로 분리되어 있습니다.

## Threads 게시 성과

게시 성과 페이지는 `/dashboard/insights`에서 확인합니다. Meta 앱에 `threads_manage_insights` 권한을 추가한 뒤 새로 발급한 long-lived token을 `THREADS_ACCESS_TOKEN`에 반영해야 합니다. 현재 MVP는 최근 20개 게시물 기준으로 조회, 좋아요, 댓글, 리포스트, 인용, 공유, 반응 합계, 반응률을 계산합니다.
...
- `components/InsightsClient.tsx`
- `components/RepliesClient.tsx`
- `components/ThreadsInsightsPanel.tsx`
- `components/ThreadsInbox.tsx`
- `lib/accounts.ts`
- `lib/postHistory.ts`
- `lib/publisher/xPublisher.ts`
- `lib/publisher/threadsPublisher.ts`
- `lib/publisher/threadsInsights.ts`
