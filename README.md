...
Threads API 연동은 `lib/publisher/threadsPublisher.ts`의 `publishToThreads`에서 처리합니다. 텍스트 게시 또는 단일 이미지 게시 기준으로 컨테이너 생성 후 publish를 호출합니다. 이미지 첨부 시 Vercel Blob에 업로드된 공개 URL을 Threads API의 `image_url`로 전달합니다. 주제 선택 시 Threads API의 `topic_tag` 파라미터로 함께 전달합니다. 설문 선택지는 Threads API의 `poll_attachment` 파라미터로 전달하며, 설문은 텍스트 게시 전용으로 이미지 첨부와 함께 사용할 수 없습니다. 타래 작성 시 첫 게시물을 발행한 뒤 추가 글을 `reply_to_id`로 이어 붙이고, 스포일러는 컨테이너 생성 단계에서 `text_entities` 또는 `is_spoiler_media`로 전달합니다.
